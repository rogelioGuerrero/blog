...
// Helper to generate IDs from heading text for anchor linking
...
const ArticleView: React.FC<Props> = ({ article, onBack, onNavigate, onPlayAudio, isPlayingCurrent, relatedArticles, settings }) => {
  ...
  const handleShare = async () => {
      ...
      if (navigator.share) {
          ...
          // Fallback for desktops - Try/Catch for permissions
          try {
              ...
              // Optionally could show a manual alert here if needed
          }
      }
  };
  ...
  const MarkdownComponents: React.ComponentProps<typeof ReactMarkdown>['components'] = {
      ...
      h1: ({ children, ...props }) => <h1 id={generateId(children)} className="scroll-mt-32 text-3xl font-serif font-bold text-slate-900 dark:text-slate-100 mt-12 mb-6 tracking-tight leading-tight" {...props}>{children}</h1>,
      h2: ({ children, ...props }) => <h2 id={generateId(children)} className="scroll-mt-32 text-2xl font-serif font-bold text-slate-800 dark:text-slate-200 mt-10 mb-4 tracking-tight leading-snug" {...props}>{children}</h2>,
      h3: ({ children, ...props }) => <h3 id={generateId(children)} className="scroll-mt-32 text-xl font-serif font-semibold text-slate-800 dark:text-slate-200 mt-8 mb-3" {...props}>{children}</h3>,
      
      // Body text: Serif, relaxed line-height, readable gray
      p: ({ children }) => <p className="mb-6 text-[1.05rem] leading-8 text-slate-700 dark:text-slate-300 font-serif antialiased text-justify sm:text-left">{children}</p>,
      
      // Lists
      ul: ({ children }) => <ul className="list-disc list-outside ml-5 mb-6 text-slate-700 dark:text-slate-300 font-serif text-[1.05rem] leading-8 marker:text-slate-400">{children}</ul>,
      ol: ({ children }) => <ol className="list-decimal list-outside ml-5 mb-6 text-slate-700 dark:text-slate-300 font-serif text-[1.05rem] leading-8 marker:text-slate-400">{children}</ol>,
      ...
      // Code blocks
      code: ({ children }) => <code className="bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 px-1.5 py-0.5 rounded text-sm font-mono text-pink-600 dark:text-pink-400">{children}</code>,
      ...
      <div className="max-w-[720px] mx-auto bg-transparent">
        ...
        <article className="article-content selection:bg-indigo-100 dark:selection:bg-indigo-900/30 selection:text-indigo-900 dark:selection:text-indigo-200">
            ...
            <div className="
                ...
                dark:[&>p:first-of-type]:first-letter:text-slate-100
